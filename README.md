...
By default, a NVMe drive in the PAX fabric cannot be enumerated by hosts. With the bind operation supported by the Switchtec Gen3 PAX switch, VFs can be bound to hosts and be enumerated by host.
...
Fabric Manager need a method to talk to the NVMe drives in the PAX fabric. Switchtec Gen3 PAX provides a special MRPC command to forward NVMe admin commands to the NVMe drives in the PAX fabric. Two communication channels are provided betwen the Fabric Manager and the PAX switch, they are the local inband PCIe channel and the remote MOE channel.

Switchtec-nvme-cli is responsible to create NVMe admin commands and deliver them to NVMe drives in PAX fabric through the special MRPC command, over either the local inband PCIe channel or the remote MOE channel.
